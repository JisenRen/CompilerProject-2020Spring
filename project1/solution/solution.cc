...
struct Func{
	...
	Func(const vector<pair<Node*, Node*> >& ker,const Json::Value& obj){
		...
		int s_in = obj["ins"].size(), s_out = obj["outs"].size();
		set<string> S;
		for (int i = 0; i < s_in; i++) 
		{
			string _ = obj["ins"][i].asString();
			signame.push_back(_);
			S.insert(_);
		}
		for (int i = 0; i < s_out; i++) 
		{
			string _ = obj["outs"][i].asString();
			if (S.find(_)==S.end())
				signame.push_back(_);		
		}
	}
...
void printRefInfo(Node *n){
	...
	for(int i=0;i<refvec.size();i++){
		...
		for(int j=0;j<refvec[i].paridx.size();j++){
			cout<<"  No."<<j<<" paridxname = "<<refvec[i].paridx[j]<<endl;
		}
		for(int j=0;j<refvec[i].idx.size();j++){
...
int main() {
	string filepath = "./cases/case";
	string temppath, outfile;
	for (int num = 1; num <= 10; num++)
	{
		temppath = filepath + myitoa(num) + ".json";
		ifstream case_file(temppath, ifstream::binary);
		Json::Value obj;
		Json::Reader reader;
		...
		L = tokens.size();
		if (L==0) continue;
		vector<Token>::iterator itt;
		vector<pair<Node*, Node*> >::iterator itk;

		for (k = 0; k < L; Kernel.push_back(make_pair(Left_root, Right_root)))
		{
			for (current = 35; tokens[k].type != Equal; k++)
				Analyze(new Terminal(tokens[k]));
			Analyze(new Terminal(tokens[k++]));
			for (current = 0; tokens[k].type != Semicolon; k++)
				Analyze(new Terminal(tokens[k]));
			Analyze(new Terminal(tokens[k++]));
		}

		Func body(Kernel,obj);
		string answer = body.getAnswer();
		outfile = "./kernels/kernel_case" + myitoa(num) + ".cc";
		fstream fout;
		fout.open(outfile, ios::out);
		fout << "#include \"../run.h\"" << endl;
		fout << answer << endl;
		fout.close();
		in_id.clear(), out_id.clear();
		tokens.clear();
		Kernel.clear();
	}
	return 0;
